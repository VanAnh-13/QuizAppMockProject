...
async function createStore(pageSize = 6): Promise<QuizExploreStore> {
  TestBed.configureTestingModule({
    providers: [
      { provide: QUIZ_CATALOG, useClass: PrototypeQuizCatalog },
      QuizExploreStore,
...
describe('QuizExploreStore', () => {
  ...
  it.each(['kieu gia tri', 'KIỂU GIÁ TRỊ', 'kiểu giá trị'])(
    ...
    async (term) => {
      const store = await createStore();
      store.applySearch(term);
      expect(store.visibleQuizzes()[0]?.id).toBe('csharp-co-ban-oop');
    },
  );
  ...
  it('normalizes the Vietnamese letter đ', async () => {
    ...
    expect(store.visibleQuizzes()[0]?.id).toBe('angular-routing-forms');
  });
  ...
  it('reports zero ranges when nothing matches', async () => {
    const store = await createStore();
    store.applySearch('không có bài quiz này');
