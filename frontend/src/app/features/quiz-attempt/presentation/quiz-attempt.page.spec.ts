...
describe('QuizAttemptPage', () => {
    ...
    beforeEach(async () => {
        ...
        const api: Partial<AttemptApi> = {
            ...
            resume: vi.fn().mockResolvedValue({
                ...start, revision: 1, pausedAt: null, lastSavedAt: null,
                remainingSeconds: 1200, answers: [],
            }),
        };
    ...
    it('keeps the quiz controls and guidance in English after resuming', async () => {
        const fixture = await createFixture();
        const store = fixture.debugElement.injector.get(QuizAttemptStore);
        const api = fixture.debugElement.injector.get(ATTEMPT_API);
        const element = fixture.nativeElement as HTMLElement;
        store.isPaused.set(true);
        fixture.detectChanges();

        element.querySelector<HTMLButtonElement>('[aria-label="Resume quiz"]')!.click();
        await fixture.whenStable();
        fixture.detectChanges();

        expect(api.resume).toHaveBeenCalledWith(start.attemptId, start.revision);
        expect(store.isPaused()).toBe(false);
        expect(element.querySelector('[aria-label="Pause quiz"]')).not.toBeNull();
        expect(element.textContent).toContain('Select one answer.');
        expect(element.textContent).toContain('Submit quiz');
        expect(element.querySelector('.attempt-status--paused')).toBeNull();
    });

    it('keeps the current attempt in the login destination after an authentication error', async () => {
        const fixture = await createFixture();
        const store = fixture.debugElement.injector.get(QuizAttemptStore);
        store.errorMessage.set('Please log in again to continue.');
        fixture.detectChanges();
        const element = fixture.nativeElement as HTMLElement;
    ...
    it('labels a shortfall against the quiz threshold and shows the required score', async () => {
        ...
        const pill = element.querySelector('.score-status-pill')!;
        expect(pill.textContent).toContain('NOT PASSED (NEEDS ≥75%)');
    ...
    it('renders a neutral result without a pass verdict when no threshold is configured', async () => {
        ...
        const pill = element.querySelector('.score-status-pill')!;
        expect(pill.textContent).toContain('NO PASSING SCORE SET');
        expect(pill.classList.contains('score-status-pill--pass')).toBe(false);
    ...
    it('replaces next button with submit button on the last question', async () => {
        ...
        expect(submitNavBtn?.textContent).toContain('Submit quiz');
    });
    ...
    it('dismisses a pending reload confirmation when the page is destroyed', async () => {
        ...
        expect(confirmation.request()?.title).toBe('Reload attempt?');
