...
internal sealed record AttemptQuizSnapshot(
    Guid Id,
    string Title,
    string? Description,
    int Duration,
    string? Image,
    double? PassedScore,
    AttemptQuestionSnapshot[] Questions)
{
        ...
        [
            .. quiz.QuizQuestions.Where(qq => qq.QuestionNavigation.IsActive)
                ...
                .Select(qq => new AttemptQuestionSnapshot(
                    qq.QuestionId,
                    qq.QuestionNavigation.Content,
                    qq.QuestionNavigation.Image,
                    qq.QuestionNavigation.Level,
                    qq.QuestionNavigation.QuestionType,
                    qq.Order,
                    [
                        .. qq.QuestionNavigation.Answers.Where(a => a.IsActive)
                            .Select(a => new AttemptAnswerSnapshot(a.Id, a.Text, a.IsCorrect))
                    ]))
        ]));
    ...
    public static Quiz Read(QuizAttempt attempt)
    {
        ...
        var quiz = new Quiz
        {
            ...
            PassedScore = snapshot.PassedScore, 
            IsActive = true
        };
        ...
        foreach (var saved in snapshot.Questions)
        {
            var question = new Question
            {
                ...
                Answers =
                [
                    .. saved.Answers.Select(answer => new Answer
                    {
...
internal sealed record AttemptQuestionSnapshot(
    Guid Id,
    string Content,
    string? Image,
    QuestionLevel? Level,
