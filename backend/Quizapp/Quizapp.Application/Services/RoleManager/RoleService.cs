    ...
    ServiceAuthorization authorization) : IRoleService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return ServiceRules.MapPage(await roles.GetListAsync(pageNumber, pageSize, ServiceRules.Search(search), cancellationToken), DtoMapping.ToDto);
    }
    ...
    public async Task UpdateAsync(Guid roleId, UpdateRoleDto request, CancellationToken cancellationToken = default)
    {
        ...
        if (string.Equals(role.RoleName, ServiceAuthorization.AdministratorRole, StringComparison.OrdinalIgnoreCase)
            && (!request.IsActive || !string.Equals(name, ServiceAuthorization.AdministratorRole, StringComparison.OrdinalIgnoreCase))
            && await roles.HasUsersAsync(roleId, cancellationToken))
            throw new BusinessRuleException("AdministratorRoleInUse", "An assigned administrator role cannot be renamed or disabled.");
