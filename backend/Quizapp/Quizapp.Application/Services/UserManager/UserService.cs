    ...
    IValidator<UpdateUserRolesDto> rolesValidator) : IUserService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return ServiceRules.MapPage(
            await users.GetListAsync(pageNumber, pageSize, ServiceRules.Search(search), cancellationToken),
            DtoMapping.ToDto);
    }
    ...
    public async Task UpdateRolesAsync(Guid userId, UpdateUserRolesDto request,
        CancellationToken cancellationToken = default)
    {
        var administrator = await authorization.RequireAdminAsync(cancellationToken);
        ...
        var missingId = request.RoleIds.Except(assignments.Select(role => role.Id))
            .FirstOrDefault();
        ...
        if (administrator.Id == userId && !assignments.Any(role =>
                string.Equals(role.RoleName, ServiceAuthorization.AdministratorRole,
                    StringComparison.OrdinalIgnoreCase)))
            throw new BusinessRuleException("SelfRoleRemoval", "You cannot remove your own administrator role.");

        var selectedIds = assignments.Select(role => role.Id)
            .ToHashSet();

        foreach (var role in user.Roles.Where(role => !selectedIds.Contains(role.Id))
                     .ToArray())
    ...
    private static void PreventSelfDeactivation(Guid administratorId, Guid targetId, bool isActive)
    {
        if (administratorId == targetId && !isActive)
            throw new BusinessRuleException("SelfDeactivation",
