...
public sealed class UserProfileDtoValidator : AbstractValidator<UserProfileDto>
{
    ...
    {
        RuleFor(dto => dto.FullName).NotEmpty().MaximumLength(FieldLimits.FullNameLength);
        RuleFor(dto => dto.PhoneNumber).MaximumLength(FieldLimits.PhoneNumberLength);
        RuleFor(dto => dto.Avatar).MaximumLength(FieldLimits.ImageUrlLength);
