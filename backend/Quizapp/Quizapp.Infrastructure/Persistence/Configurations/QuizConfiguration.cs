...
internal sealed class QuizConfiguration : IEntityTypeConfiguration<Quiz>
{
    public void Configure(EntityTypeBuilder<Quiz> entry)
    {
        ...
        entry.Property(quiz => quiz.Title)
            .HasMaxLength(FieldLimits.QuizTitleLength)
            .IsRequired();

        entry.Property(quiz => quiz.Description)
            .HasMaxLength(FieldLimits.ContentLength);

        entry.Property(quiz => quiz.Image)
        ...
        entry.HasMany(quiz => quiz.Questions)
            ...
            .UsingEntity<QuizQuestion>(assignment =>
            {
                assignment.HasKey(value => value.Id);

                assignment.HasIndex(value => new { value.QuizId, value.QuestionId })
                    .IsUnique();
