...
internal sealed class ServiceTestContext : IDisposable
{
    ...
    public ServiceTestContext()
    {
        ...
        var adminRole = new Role { Id = Guid.NewGuid(), RoleName = ServiceAuthorization.AdministratorRole, IsActive = true };
        ...
        Services.Configure<JwtOptions>(options =>
        {
            ...
            options.SigningKey = Convert.ToBase64String(System.Security.Cryptography.RandomNumberGenerator.GetBytes(32));
        });
    }
...
internal sealed class MemoryQuestions() : MemoryRepository<Question>(question => question.Id, question => question.Content), IQuestionRepository;
...
internal abstract class MemoryRepository<T>(Func<T, Guid> id, Func<T, string> text) : IRepository<T> where T : class
{
    ...
    public Task<PagedResultDto<T>> GetListAsync(int pageNumber, int pageSize, string? search, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        var query = Rows.Where(row => search is null || text(row).Contains(search, StringComparison.OrdinalIgnoreCase))
            .OrderBy(text).ThenBy(id).ToArray();
...
internal sealed class MemoryUsers() : MemoryRepository<User>(user => user.Id, user => user.Username), IUserRepository
{
    ...
    {
        cancellationToken.ThrowIfCancellationRequested();
        return Task.FromResult(Rows.SingleOrDefault(user => string.Equals(user.Username, username, StringComparison.OrdinalIgnoreCase)));
    }

    public Task<bool> UsernameExistsAsync(string username, Guid? excludedId, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        return Task.FromResult(Rows.Any(user => user.Id != excludedId && string.Equals(user.Username, username, StringComparison.OrdinalIgnoreCase)));
    }

    public Task<bool> EmailExistsAsync(string email, Guid? excludedId, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        return Task.FromResult(Rows.Any(user => user.Id != excludedId && string.Equals(user.Email, email, StringComparison.OrdinalIgnoreCase)));
    }
}

internal sealed class MemoryRoles(MemoryUsers users) : MemoryRepository<Role>(role => role.Id, role => role.RoleName), IRoleRepository
{
    public Task<bool> NameExistsAsync(string name, Guid? excludedId, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        return Task.FromResult(Rows.Any(role => role.Id != excludedId && string.Equals(role.RoleName, name, StringComparison.OrdinalIgnoreCase)));
    }

    public Task<IReadOnlyList<Role>> GetByIdsAsync(IReadOnlyCollection<Guid> ids, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        return Task.FromResult<IReadOnlyList<Role>>(Rows.Where(role => ids.Contains(role.Id)).ToArray());
    }
