...
public class DtoBuilderTests
{
    ...
    {
        var builder = new CreateQuizDto.Builder().WithTitle("C# basics").WithDuration(30);

        var first = builder.Build();
        var second = builder.WithTitle("Advanced C#").WithPassedScore(120).Build();
        ...
        Assert.Equal("Advanced C#", second.Title);
        Assert.Equal("Advanced C#", builder.Build().Title);
    }
    ...
    public void Submission_builder_snapshots_collections_at_assignment_and_each_build()
    {
        ...
        var builder = new SubmitAnswerDto.Builder().WithQuestionId(questionId).WithAnswerIds(input);
        input.Clear();
        ...
        Assert.Single(submissionBuilder.Build().Answers);
        Assert.Empty(new SubmitQuizDto.Builder().Build().Answers);
    ...
    public void Unset_values_use_dto_defaults_and_can_be_assigned_after_creation()
    {
        ...
        Assert.False(answer.Build().IsCorrect);

        var question = new CreateQuestionDto.Builder().WithContent("A question");
        Assert.Equal((QuestionLevel)0, question.Build().Level);
        Assert.Equal(QuestionLevel.Easy, question.WithLevel(QuestionLevel.Easy).Build().Level);
    }
    ...
    public void Builders_preserve_defaults_and_leave_business_rules_to_validators()
    {
        var assignment = new AddQuestionToQuizDto.Builder()
            .WithQuizId(Guid.NewGuid()).WithQuestionId(Guid.NewGuid()).Build();
        Assert.Equal(QuizQuestion.FirstOrder, assignment.Order);

        var invalidQuiz = new CreateQuizDto.Builder()
            .WithTitle("Quiz").WithDuration(-1).WithPassedScore(-1).Build();
        Assert.False(new CreateQuizDtoValidator().Validate(invalidQuiz).IsValid);

        var legacyQuiz = new QuizDto.Builder().WithTitle("Legacy quiz").WithDuration(30).Build();
    ...
    {
        var profile = new UserProfileDto.Builder().WithFullName("Student Name").Build();
        var registration = new RegisterDto.Builder().WithUsername("student")
            .WithEmail("student@example.com").WithPassword("test-only-password")
            .WithConfirmPassword("test-only-password").WithProfile(profile).Build();
        ...
        var authentication = new AuthResponseDto.Builder().WithToken("test-only-token")
            .WithUserDto(new UserDto.Builder().WithUsername("student").WithEmail("student@example.com").Build())
            .Build();
        ...
        var quiz = new QuizForAttemptDto.Builder().WithTitle("Quiz")
            .WithQuestions([new QuestionForAttemptDto.Builder().WithContent("Question")
                .WithAnswers([new AnswerOptionDto.Builder().WithText("Option").Build()]).Build()])
            .Build();
    ...
    public void History_detail_builder_includes_inherited_fields_and_nullable_legacy_data()
    {
        ...
        var result = new UserAnswerResultDto.Builder().WithQuestionContent("Legacy question")
            .WithResponseText("An explanation").Build();
        var history = new QuizAttemptDetailDto.Builder().WithId(attemptId).WithQuizId(quizId)
            .WithQuizTitle("History quiz").WithSubmittedAt(submittedAt).WithScore(7.5)
            .WithAnswers([result]).Build();
        ...
        Assert.Equal("An explanation", Assert.Single(history.Answers).ResponseText);
