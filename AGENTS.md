...
The application is a .NET 10 backend with controllers and services for authentication, user/role management, quiz management, quiz taking, and history. JWT settings are validated before startup, and authentication checks the current user's security stamp. Starting an attempt uses POST and persists its owner, deadline and quiz snapshot. Progress can be saved, paused (freezing time), and resumed on the same attempt. The frontend triggers submission; expired attempts grade only saved answers. Progress writes use revisions to reject stale updates. See backend/docs/attempt-progress.md for the contract. Development OpenAPI/Swagger is available. The Angular frontend is a prototype: explore/details use the public quiz catalog API, while the attempt screen remains local until authentication is integrated.
